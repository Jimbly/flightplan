...
Transport.prototype.sudo = function(command, options) {
  ...
  command = format("sudo -u %s " + (this._options.noninteractive ? "" : "-i ") +  "bash -c '%s'", user, command);
 ...
 */
Transport.prototype.noninteractive = function() {
  this._options.noninteractive = true;
};
